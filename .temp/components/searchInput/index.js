...
import Taro, { setStorageSync as _setStorageSync, removeStorageSync as _removeStorageSync } from "@tarojs/taro-h5";
import './index.scss';
...
class SearchInput extends Taro.Component {
  ...
  searchTopic = event => {
    ...
    event.detail.value && that.state.historyList.indexOf(event.detail.value) === -1 && that.setState({
      ...
      _setStorageSync('searchHistory', { data: that.state.historyList });
    });
    Taro.navigateTo({
  ...
  clearAll = () => {
    let that = this;
    _removeStorageSync('searchHistory');
    that.setState({
      historyList: []
    });
  };
  ...
  render() {
    ...
    return <View className="search-component">
        ...
        <View className={'search-mask ' + (show ? 'show' : 'hide')}>
          <View className="search-input-wrap">
            ...
            <Input className="search-mask-input market-class" type="text" confirmType="search" focus={show} value={value} autoFocus={show} placeholderStyle="color:#cdcdcd" placeholder="搜索想知道内容" onConfirm={this.searchTopic} />
            <Text className="search-mask-cancel" onClick={hideMask}>
              取消
            </Text>
          </View>
          {historyList.length > 0 && <View className="search-history">
              <View className="search-history-title">搜索历史</View>
              {historyList.map((item, index) => {
            return <View className="search-item" key={index.id}>
                    <Image className="search-icon-time" src={require('../../assets/images/time.png')} />
                    <Text className="search-item-text">{item}</Text>
                    <Image className="search-icon-del" data-index={index} onClick={this.clearItem} src={require('../../assets/images/del-item.png')} />
                  </View>;
          })}
              {historyList.length > 1 && <View className="search-clear-all" onClick={this.clearAll}>
